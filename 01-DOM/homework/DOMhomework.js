...
// Tu codigo acá:
let toDoItems = []; //creo un array vacío

// En la página 'index.html' hay un elemento span cuyo texto es 'Aplicación creada por:'.
// Usando querySelector seleccionar dicho span por su id ('createdBy') y luego usando innerHTML
// agregar tu nombre al final del texto actual. Ej: 'Aplicación creada por Franco'
// Tu código acá:
var span = document.getElementById("createdBy"); // creo una variable span q contenga el id created by
span.innerHTML = span.innerHTML + ' Fabian Calle'; // le agrego a la propiedad innerHTML mi nombre
...
function ToDo(description) {// agrego un parametro llamado description
  // Tu código acá:
  this.description = description;// agrego las propiedades solicitadas
  this.complete = false;
}
...
ToDo.prototype.completeToDo = function () {
  this.complete = !this.complete; // paso a true a complete
};
...
function buildToDo(todo, index) {
  ...
  let toDoText = document.createElement('span');// le creo un span toDoText
  toDoText.innerHTML = todo.description;// le agrego el description al innerHTML
  toDoText.id = index;// le doy el id de index

  if (todo.complete) toDoText.className = 'completeText';// si complete es true le doy el nombre completText


  toDoShell.appendChild(toDoText);// agrego a toDoText como hijo de toDoShell
...
function buildToDos(toDos) {
  // Tu código acá:
  //recorre todos los elementos y los guarda en un array
  let arr = toDos.map(function (todo, i) { //creo una variable para recorrer los elementos e index 
    return buildToDo(todo, i);//regresa y retorna la funcion anterior para recorrerla 
  });
  return arr;// regreso el array
}
...
function displayToDos() {
  // Tu código acá:
  let toDoContainer = document.getElementById('toDoContainer'); //asigno el elemento de toDoContainer a una variable
  toDoContainer.innerHTML = ""; //inicia vacio
  let build = buildToDos(toDoItems);//asigno a una variable la funcion buildtodo con el parametro del array todoitems

  build.map(function(element){//mapeo el array para buscar los elementos
    toDoContainer.appendChild(element);// agrego los elementos al contenedor
  });
...
function addToDo() {
  // Tu código acá:
let input = document.getElementById('toDoInput');//creo una variable con el id de todoinput
if(input.value !== ''){// en caso de que no este vacio
  let toDo = new ToDo(input.value);// creo un nuevo todocon el parametro del value
  toDoItems.push(toDo);//lo pusheo a mi array
  input.value = '';//vuelvo a ponerlo en blanco 
  displayToDos();
}
...
// Tu código acá:
let add = document.getElementById('addButton');//selecciono el elemento con id addbutton
add.addEventListener('click', addToDo);// le creo el eventlistener
...
function completeToDo(event) {
  // DESCOMENTAR LA SIGUIENTE LINEA
   const index = event.target.id;//con el evento de click sobre el elemento llamo a completetodo
  // Tu código acá:
